...
export const compileLatex = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    // The temporary paths are generated by this server, not supplied by the client.
    const command = `pdflatex -no-shell-escape -interaction=nonstopmode -halt-on-error -output-directory=${tempDir} ${texFilePath}`;
    
    try {
      await execAsync(command, { timeout: 10000, maxBuffer: 1024 * 1024 }); // 10s timeout, 1 MB log limit
